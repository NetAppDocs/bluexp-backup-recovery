...
BlueXP Connector also interacts with Azure NetApp Files (ANF) and SnapCenter Plug-in for SAP HANA running on SAP HANA database hosts.
...
+
For any user-initiated request, the Cloud Backup for Applications UI communicates with the BlueXP SaaS which upon validating the request processes the same. If the request is to run a workflow such as a backup or restore, the SaaS service initiates the workflow and where required, forwards the call to the BlueXP Connector. The Connector then communicates with ANF for NetApp ONTAP, Amazon FSx for NetApp ONTAP or Cloud Volumes ONTAP, SnapCenter Plug-in for Oracle, and SnapCenter Plug-in for SAP HANA as part of running the workflow tasks.
+
The Connector can be deployed in the same VPC as that of the Oracle databases and SAP HANA databases, or in a different one. If the Connector and Oracle and SAP HANA databases are on different network, you should establish a network connectivity between them.
