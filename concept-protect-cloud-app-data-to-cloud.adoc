...
* The network interface IP addresses of Amazon FSx for NetApp ONTAP or Cloud Volumes ONTAP needs to be unique in the Cloud Manager account and region.
* Backup limitations:
...
** Both in-place and connect-and-copy restore methods are used if the database is shared with multiple disk groups. This mixed strategy is not supported and the restore operation fails. The database will be left in mounted state and you need to manually bring the database to open state.
+
