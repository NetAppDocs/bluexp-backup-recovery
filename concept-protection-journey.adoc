...
NOTE: If you are planning to create custom policies when using the cascading architecture, you'll need to create the backup to object policies on the secondary system where the replicated volumes will be created. If you're using the default policies, then you are all set. 

You can create backup to object storage policies in the BleXP backup and recovery UI. See the section for link:task-manage-backups-ontap.html#add-a-new-backup-policy[adding a new backup policy] for details. You'll need to create Snapshot and Replication policies using System Manager or the ONTAP CLI.
