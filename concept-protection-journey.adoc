...
A _cascade_ architecture transfers the Snapshot copy to the destination storage system first, and then that system transfers the copy to the backup object in the cloud. This method has less effect on the performance of the source storage system because it transfers the copy only once.
