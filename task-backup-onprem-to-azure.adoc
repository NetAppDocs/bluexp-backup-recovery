...
Cloud Backup is supported in Azure Government regions when the Connector is deployed in the cloud - not when it's installed in your premises or in a dark site. Additionally, you must deploy the Connector from the Azure Marketplace. You can’t deploy the Connector in a Government region from the Cloud Manager SaaS website. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-launching-azure-mktp.html[Learn how to create the Connector in an Azure Government region].
