...
keywords: back up, backup, backup virtual machine data, gcp, google cloud provider, cloud backup, backup datastores
summary: Back up your datastores from on-premises ONTAP to Google Cloud.
---

= Back up datastores to Google Cloud
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
You can back up one or more datastores to Google Cloud by integrating the SnapCenter Plug-in for VMware vSphere with BlueXP. This will help the VM administrators to easily and quickly back up and archive data for storage efficiency and accelerate cloud transition.
...
. Select *Google Cloud Provider* to configure it as the cloud provider.
