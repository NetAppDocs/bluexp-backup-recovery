...
You'll need to sign up for a cloud provider subscription for the storage space where your backups will be located. And you'll need to set up an account. Then you'll enter your cloud provider credentials and select the storage class that you want to use. Cloud Backup creates the cloud bucket when you activate backup for the cluster. The information required for AWS archival storage is shown below.
