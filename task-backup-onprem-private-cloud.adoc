...
[.lead]
Complete a few steps to get started backing up volume data from your on-premises ONTAP systems to a secondary storage system and to object storage in your NetApp StorageGRID systems.
...
[role="quick-margin-para"]
If you already have a Connector deployed in your StorageGRID or on your premises, then you're all set. If not, then you'll need to create a BlueXP Connector to back up ONTAP data to StorageGRID. You'll also need to customize network settings for the Connector so that it can connect to StorageGRID.
...
[role="quick-margin-para"]
Set up permissions for the Connector to create and manage the StoageGRID bucket. You'll also need to set up permissions for the on-premises ONTAP cluster so it can read and write data to the bucket.
...
When you back up data to StorageGRID, a BlueXP Connector must be available on your premises. You'll either need to install a new Connector or make sure that the currently selected Connector resides on-prem. The Connector can be installed in a site with or without internet access.
