...
If you import resources from SnapCenter, you might encounter some differences between SnapCenter policies and those used in BlueXP backup and recovery. See link:reference-policy-differences-snapcenter.html[Policy differences between SnapCenter and BlueXP backup and recovery^].
...
You can create policies that govern your local snapshots, replications to secondary storage, and backups to object storage. Part of your 3-2-1 strategy involves creating a snapshot copy of the MS SQL Server instances or databases on the *primary* storage system. 
