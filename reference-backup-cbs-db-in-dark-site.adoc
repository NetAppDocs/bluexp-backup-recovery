...
This data is backed up once per day at midnight, and a maximum of 7 copies of each file are retained. In case of single connector serving multiple WEs, DBs backup will be located in the bucket of the first activated WE. 
...
+
This file is named `CBS_DB_Backup_<day>_<month>_<year>`
* Indexed Catalog backup zip file
+
This file is named `Indexed_Catalog_DB_Backup_<day>_<month>_<year>`
...
. Copy the MySQL backups from the backup location to the new Connector host.
...
. Copy the Indexed Catalog backup zip file from the backup location to the new Connector host in the "/opt/application/netapp/cbs" folder.
