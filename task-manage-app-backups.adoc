...
* For Oracle databases, both data and log backups, system change number (SCN) for each backup, end date for each backup are listed. You can select only the data backup and restore the database to original or alternate location. You can mount the data backup and log backup to alternate location.
