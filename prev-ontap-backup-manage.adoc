...
[.lead]
With BlueXP backup and recovery, manage backups for your Cloud Volumes ONTAP and on-premises ONTAP systems by changing the backup schedule, enabling/disabling volume backups, pausing backups, deleting backups, force deleting backups, and more. This includes all types of backups, including snapshot copies, replicated volumes, and backup files in object storage. You can also unregister BlueXP backup and recovery. 
...
+
image:screenshot_backup_delete_all_backups-working-env.png[A screenshot that shows the Delete All Backups option]

. In the confirmation dialog box, enter the name of the working environment. 
