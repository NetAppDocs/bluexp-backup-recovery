...
. Specify the SID of the clone.
. (Optional) Specify the database credentials.
* Database credential: If the OS User authentication is disabled, you should specify the credentials of the database. These credentials can be used later to log into the database.
* ASM credential: If the OS user authentication is disabled on the target host, you should specify the credential of the ASM instance running on the target host.
. Click *Next*.
. Click *Clone*.
|===
