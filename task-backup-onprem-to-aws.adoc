...
Refer to the https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user.html[AWS Documentation: Creating a Role to Delegate Permissions to an IAM User^] for details.
...
Activate backups at any time directly from the on-premises working environment.

A wizard takes you through the following major steps: 

* <<Select volumes>>
* <<Define the backup strategy>>, where you choose whether you want local Snapshots, replication, and backup. Then, you choose the architecture, local Snapshot policy, replication target and policy, backup provider, encryption, networking, backup policy, and export options. 
* <<Review your selections>> 

You can also <<Show the API commands>>.  

=== Start the wizard 

.Steps 

. Access the Activate backup and recovery wizard. From the BlueXP canvas, select the working environment and click *Enable > Backup Volumes* next to the Backup and recovery service in the right-panel.
+
...
+ 
The Introduction page of the wizard shows the protection options including local Snapshots, replication, and backups. 


. Do one of the following: 

* If you already have a BlueXP Connector, you're all set. Just select *Next*. 
* If you don't already have a BlueXP Connector, the *Add a Connector* option appears. Refer to ZZZ for details about how to add a Connector. 
* If you don't have a BlueXP working environment set up, you first need to set one up. Refer to ZZZ for details about how to set up a working environment. 
* If you have multiple working environments, select the one that you want to protect. 

=== Select volumes 

Next, choose the volumes you want to protect. You can choose to protect FlexVol or FlexGroup volumes; however, you cannot select a mix of these for the working environment you chose to protect.  
...
.Steps 
. In the Select Volumes page of the wizard, select the volume or volumes you want to protect. 
+
NOTE: After you select the Volume style of FlexVol or FlexGroup, then you can select all volumes. 
+
* To back up all existing volumes and any volumes added in the future, check one volume first and then check the box in the title row. (image:button_backup_all_volumes.png[]).
+
...
* Whether you want one or all of local Snapshots, replication, and backup
* Architecture
...
* Backup information (provider, encryption, networking, backup policy, and export options). 
...
. *Local Snapshot*: Choose the existing default Snapshot policy. 
+
...
+
* *Replication target*: Set the destination working environment, SVM, aggregates and volume name. 
* *Replication policy*: Choose an existing policy. 

. *Back up to Object*: If you selected *Backup*, set the following options: 
+
* *Provider*: Select Amazon Web Services and enter the provider details. Enter the AWS region where the backups will be stored.

+
The access key and secret key are for the IAM user you created to give the ONTAP cluster access to the S3 bucket.


* *Bucket*: Either choose an existing S3 bucket or create a new one. 
...
* *Networking*: Choose either IPspace, the default, or Private Endpoint. Private Endpoint is disabled by default.
...
. Optionally check the box to *Automatically synchronize the Snapshot policy labels with the replication and backup policy labels*. This takes the Snapshot policy labels and assigns them to the replication and backup labels. 
. Select *Activate Backup*. 


.Result 
BlueXP backup and recovery starts taking the initial backups of your volumes.
...
You might want to display and optionally copy the API commands used in the Activate backup and recovery wizard. 
