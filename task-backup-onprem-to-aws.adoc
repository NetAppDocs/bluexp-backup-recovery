...
* When you use a fanout backup architecture, the following settings must be configured on the source system. 
...
//This section uses an Include for all providers. It includes "ONTAP version requirements" (REMOVED for REWORK), "Cloud Volumes ONTAP networking requirements", and "On-premises ONTAP networking requirements"  NEED TO FIX INCLUDE TO remove 1st section on ONTAP version requirements. 
