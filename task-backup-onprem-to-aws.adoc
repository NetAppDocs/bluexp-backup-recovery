...
You'll need to prepare your source on-premises ONTAP system and any secondary on-premises ONTAP (or Cloud Volumes ONTAP systems - for the CVO topics).

Preparing your ONTAP clusters involves the following steps: 

* Discover the ONTAP systems in BlueXP
* Verify ONTAP requirements
* Verify cluster networking requirements

// This section used to use an include along with the 2 subsections of "Discover your ONTAP cluster in BlueXP" and "Verify ONTAP requirements"
...
(3 current bullets from the INCLUDE, plus 1 new bullet that you have in the replication section that has the link to "ONTAP versions for SnapMirror")
WAS FROM INCLUDE, but I pulled it out of include
...
=== Verify ONTAP networking requirements for creating object backups
(current section with 7 bullets)
...
===  Verify ONTAP networking requirements for creating replicated volumes
(2 sections you already have for Replication for CVO and on-prem systems)

//This section uses an Include for all providers. It includes "ONTAP version requirements", "Cloud Volumes ONTAP networking requirements", and "On-premises ONTAP networking requirements"  NEED TO FIX INCLUDE TO remove 1st section on ONTAP version requirements. 
