...
Cloud Backup is supported in GovCloud regions when the Connector is deployed in the cloud - not when it's installed in your premises or in a dark site. Additionally, you must deploy the Connector from the AWS Marketplace. You can’t deploy the Connector in a Government region from the Cloud Manager SaaS website. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-launching-aws-mktp.html#create-the-connector-in-an-aws-government-region[Learn how to create the Connector in an AWS GovCloud region].
...
. Confirm that the following S3 permissions (from the latest https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions-aws.html[Cloud Manager policy^]) are part of the IAM role that provides the Connector with permissions.
+
[source,json]
{
          "Sid": "backupPolicy",
...
+
* To back up all existing volumes and any volumes added in the future, check the box "Back up all existing and future volumes...". We recommend using this option so that all volumes will be backed up and you'll never have to remember to enable backups for new volumes.
