...
+
NOTE: The user token from NetApp Cloud Central has an expiration date. The API response includes an "expires_in" field that states when the token expires. To refresh the token, you'll need to call this API again. 
...
You'll see that we use the Cloud Central account ID "account-DpTFcxN3", BlueXP Connector ID "iZwFFeVCZjWnzGlw8RgD0QQNANZvpP7Iclients", and user token "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6Ik5rSXlPVFUzUWpZek1E…y6nyhBjwkeMwHc4ValobjUmju2x0xUH48g" in this command.
