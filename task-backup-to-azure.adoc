...
To use the Cloud Backup Search & Restore functionality, you need to have specific permissions in the role for the Connector so that it can access the Azure Synapse Workspace and Data Lake Storage Account. Also, port 1433 must be open for communication between the Connector and the Azure Synapse SQL services.
+
See the permissions below, and follow the steps if you need to modify the policy.
+
.Before you start
+
