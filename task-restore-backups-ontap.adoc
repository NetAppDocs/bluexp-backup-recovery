...
* Folder-level restore is not currently supported if the backup file has been configured with DataLock & Ransomware. In this case, you can restore the entire volume from the backup file and then access the folder and files you need.
* Folder-level restore is not currently supported if the backup file resides in archival storage. In this case, you can restore the folder from a newer backup file that has not been archived, or you can restore the entire volume from the archived backup and then access the folder and files you need.
* The High restore priority is not supported when restoring data from Azure to StorageGRID systems.
====
