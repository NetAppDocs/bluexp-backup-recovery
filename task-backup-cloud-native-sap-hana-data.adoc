...
You can add Non-Data Volumes for storing non-data files belonging to a system. These are added separately from Data Volumes so that their backups can be scheduled based on the specific requirements of the user.  These backups can be used to recover from situations where non-data files belonging to a system must be restored.
