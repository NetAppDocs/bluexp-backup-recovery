...
Restore VMware workloads using the Restore option. You can search for the snapshot using filters. 
...
+
A list of snapshots that match your search criteria appears.
+
...
. Select *Done* or *Next* to continue to the Restore options page.
+
...
* *Enable quick restore*: Select this to perform a quick restore operation. This option is available only if the source system from which the cloud backup was created was running ONTAP 9.13.0 or greater or to an on-premises ONTAP system running ONTAP 9.14.1.
...
*** *Operational*, but unavailable for restoring additional transaction logs. This brings the database back online after transaction log backups are applied. (Applies only to object storage to an alternate location.)
*** *Non-operational*, but available for restoring additional transaction logs. Maintains the database in a non-operational state after the restore operation while restoring transaction log backups. This option is useful for restoring additional transaction logs. (Applies only to object storage to an alternate location.)
*** *Read-only mode* and available for restoring additional transaction logs. Restores the database in a read-only mode and applies transaction log backups. (Applies only to object storage to an alternate location.)
*** *Postscript*: Enter the full path for a script that should be run after the restore operation and any arguments that the script takes.
...
. On the VMware page, select the *Databases* tab. 
+
image:screen-vm-inventory-managed.png[Inventory screenshot for BlueXP backup and recovery]
. On the XX tab, select the database that shows a "Protected" status indicating that there's a backup that you can restore.
+
