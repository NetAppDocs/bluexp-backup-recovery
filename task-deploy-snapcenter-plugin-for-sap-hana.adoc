...
After installing the plug-in, you should add the SAP HANA database host. <<Add SAP HANA database hosts, Learn more>>.
...
. Copy _snapcenter_linux_host_plugin_scs.bin_ to _/home/<non root user (sudo)>/.sc_netapp_ path for each of the SAP HANA database hosts either using scp or other alternate methods.
. Log into the SAP HANA database host using the non-root (sudo) account.
. Change directory to "/home/<non root user>/.sc_netapp/" and run the following command to enable execute permissions for the binary.
