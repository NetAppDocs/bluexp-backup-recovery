...
+
`docker exec -it cloudmanager_scs_cloud curl -ik https://<Plug-in Host FQDN/IP>:<Plugin Port>/getVersion --cert /config/client/certificate/certificate.pem --key /config/client/certificate/key.pem`
+
Plug-in host is the fully qualified domain name (FQDN) or the IP address of the host.
