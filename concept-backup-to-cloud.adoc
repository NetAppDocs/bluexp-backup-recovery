...
NOTE: DOCUMENTATION ABOUT PROTECTING MICROSOFT SQL SERVER, KUBERNETES, and VMWare WORKLOADS IS PROVIDED AS A TECHNOLOGY PREVIEW.  With this preview offering, NetApp reserves the right to modify offering details, contents, and timeline before General Availability.   
...
** Back up VMware workloads to Amazon Web Services S3, Microsoft Azure Blob, Google Cloud Platform, and StorageGRID and restore VMware workloads back to the on-premises VMware host. 
