...
[role="quick-margin-para"]
Set up permissions for the Connector so it can manage the ONTAP S3 bucket. You'll also need to set up permissions for the source on-premises ONTAP cluster so it can read and write data to the ONTAP S3 bucket.
...
[role="quick-margin-para"]
Select the source working environment and click *Enable > Backup Volumes* next to the Backup and recovery service in the right-panel. Then follow the setup wizard to select the Snapshot, replication, and backup to object policies that you'll use and the volumes you want to back up.
...
When the Connector and source on-premises ONTAP system are installed in an on-premises location without internet access (a "private" mode deployment), the ONTAP S3 system must be located in the same on-premises data center. 
...
+
`"Resource": ["arn:aws:s3:::backup-1*"],`
`"Resource": ["arn:aws:s3:::backup-2*"]`
+
...
* The volumes you select must have the same SnapLock setting as well. All volumes must have SnapLock Enterprise enabled or have SnapLock disabled. (Volumes with SnapLock Compliance mode aren't currently supported.)
====
...
* Protection options: Whether you want one or all of the backup options: local Snapshots, replication, and backup to object storage
