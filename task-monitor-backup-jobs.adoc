...
summary: You can monitor the status of local snapshots, replications, and backup to object storage jobs that you initiated or that are scheduled, and restore jobs that you initiated. You can see the jobs that have completed, are in progress, or failed so you can diagnose and fix problems. Using the BlueXP Notification Center, you can enable notifications to be sent by email so you can be informed of important system activity even when you're not logged into the system. Using the BlueXP Timeline, you can see details of all actions initiated via the UI or API.
---
...
[.lead]
You can monitor the status of local Snapshots, replications, and backup to object storage jobs that you initiated or that are scheduled, and restore jobs that you initiated. You can see the jobs that have completed, are in progress, or failed so you can diagnose and fix problems. Using the BlueXP Notification Center, you can enable notifications to be sent by email so you can be informed of important system activity even when you're not logged into the system. Using the BlueXP Timeline, you can see details of all actions initiated via the UI or API.
...
. To show additional columns (Working Environment, SVM, User Name, Workload, Policy Name, Snapshot Label, Protection Type, Target Working Environment for replication jobs), select image:button_plus_sign_round.png[the plus sign button].
...
| Local Snapshot | BlueXP backup and recovery scheduled Snapshot creation job failure | Error | Yes
| Replication | BlueXP backup and recovery scheduled replication job failure | Error | Yes
