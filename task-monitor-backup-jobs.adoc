...
You can view details such as backup job type (scheduled or on-demand), SnapMirror backup type (initial or periodic) start and end times, duration, amount of transferred data from working environment to object storage, average transfer rate, backup policy name, retention lock enabled, ranswomware scan performed, backup source details, and backup target details. 
...
//NEED NEW JOB DETAILS SCREEN

.Steps 
. Select the *Job Monitoring* tab.
. Select the name of the job. 
+
In the Job Details page, you can see all the running sub-jobs that are needed to complete the main job.
...
. Locate the .CSV file in your download directory. 
