...
// tag::whats-new[]
== 5 October 2023 
This release of BlueXP backup and recovery includes several updates. 

=== Support for on-demand quick restore
BlueXP backup and recovery now offers the ability to perform an on-demand, quick restore, also known as a _lazy restore_, for Cloud Volumes ONTAP data. This process restores the metadata from a cloud Snapshot to an ONTAP volume. The on-demand restore is typically faster than a full volume restore. 

The bucket source Snapshot can be from ONTAP 9.8 or later while the destination should be Cloud Volumes ONTAP 9.13.0 or later. 

The restore volume becomes available to serve data immediately. 

https://docs.netapp.com/us-en/bluexp-backup-recovery/task-restore-backups-ontap.html[Learn more about restoring data].

=== 3-2-1 Policy creation during backup activation
Previously, custom policies had to be created before you initiated a Snapshot, replication, or backup. Beginning with ONTAP 9.8 P5, you can now create a policy during the backup activation process using the BlueXP backup and recovery UI.  
...
The BlueXP backup and recovery Job Monitor previously monitored scheduled volume-to-object-store backup and restore jobs but not scheduled local Snapshots and replication backup and restore jobs via UI or API. 
...
The BlueXP Timeline now reports all modification actions made by users.  
