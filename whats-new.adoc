...
With this release, you can now enable or disable ransomware scans on the latest Snapshot copy by using the option on the Advanced Settings page. If you enable it, scans are performed weekly by default. You can change that schedule to days or weeks or disable it, saving costs.  

Refer to the following information for details: 

* link:task-manage-backup-settings-ontap.html[Manage backup settings]

* link:task-create-policies-ontap.html[Manage policies for ONTAP volumes]

* link:concept-cloud-backup-policies.html[Backup-to-object policy settings]
...
//Previously, BlueXP supported Red Hat Enterprise Linux versions 9.0, 9.1, and 9.2. 
