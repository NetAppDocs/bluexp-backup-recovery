...
[.lead]
Back up VMware VMs, datastores, and applications on VMs from on-premises ONTAP systems to Amazon Web Services or StorageGRID to ensure that your data is protected. Backups are automatically generated and stored in an object store in your public or private cloud account. 
...
* Create protection groups to manage the backup and restore operations for a set of workloads.  See link:br-use-vmware-protection-groups.html[Create and manage protection groups for VMware workloads with BlueXP backup and recovery] for more information.
