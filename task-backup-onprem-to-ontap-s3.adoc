...
Choose the volumes you want to protect. You can choose to protect FlexVol or FlexGroup volumes; however, you cannot select a mix of these types for the working environment you chose to protect.  

[NOTE] 
====
* You can activate a backup only on a single FlexGroup volume at a time. 
* The volumes you select must have the same SnapLock setting as well. All selected volumes must have SnapLock Enterprise enabled or have SnapLock disabled. (Volumes with SnapLock Compliance mode aren't currently supported.)
====
