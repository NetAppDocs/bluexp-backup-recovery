...
[.lead]
Restore database workloads from snapshot copies, a workload backup replicated to secondary storage, or from backups stored in object storage.
...
Restore database workloads using the Inventory page.
...
. Choose the host where the resouce that you want to restore is located. 
...
. Continue with the steps for the restore option you want to use, described above.
+
image:screen-br-restore-options2.png[A screenshot showing the Restore options.]
