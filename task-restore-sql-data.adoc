...
a|
Want to restore the database to the same SQL server host where the backups were created
a|
Select *Restore the database to the same host where the backup was created*.
a|
Want to restore the database to a different SQL server in the same or different host where backups were created
a|
Select *Restore the database to an alternate host*.
