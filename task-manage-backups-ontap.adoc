...
[NOTE]
====
* The _Archival Policy_ can be set only during Cloud Backup activation. You can't edit an archival policy after Cloud Backup is enabled.
* _DataLock and Ransomware Protection_ can be set only during Cloud Backup activation. You can't edit the DataLock setting after Cloud Backup is enabled.
====
...
[NOTE]
====
* The _Archival Policy_ can be set only during Cloud Backup activation. You can't add a policy with a different archival policy after Cloud Backup is enabled.
* _DataLock and Ransomware Protection_ can be set only during Cloud Backup activation. You can't add a policy with a different DataLock setting after Cloud Backup is enabled.
====

//
//NOTE: If you enabled _DataLock and Ransomware Protection_ in the initial policy when activating Cloud Backup for this cluster, any additional policies you create must be configured with the same DataLock setting (Enterprise or Compliance). And if you did not enable _DataLock and Ransomware Protection_ when activating Cloud Backup, you can't create new policies that use DataLock.
