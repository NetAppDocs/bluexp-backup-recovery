...
+
The icon is blue when that backup type is activated, and it is grey when the backup type is inactive. You can hover your cursor over each icon to see the backup policy that is being used, and other pertinent information for each type of backup.
...
== Change the backup policy assigned to existing volumes
