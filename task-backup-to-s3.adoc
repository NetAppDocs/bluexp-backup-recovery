...
* The IAM role that provides the BlueXP Connector with permissions includes S3 permissions from the latest https://mysupport.netapp.com/site/info/cloud-manager-policies[BlueXP policy^].
...
* Add the destination AWS account credentials in BlueXP
* Add the permission "s3:PutBucketPolicy" to the IAM role that provides BlueXP with permissions
...
+
If you want to use a different AWS account for your backups, you must add the destination AWS account credentials in BlueXP, and add the permission "s3:PutBucketPolicy" to the IAM role that provides BlueXP with permissions.
