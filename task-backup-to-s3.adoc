...
* The IAM role that provides the Cloud Manager Connector with permissions includes S3 permissions from the latest https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions.html[Cloud Manager policy^].
...
* Add the permissions "s3:PutBucketPolicy" and "s3:PutBucketOwnershipControls" to the IAM role that provides Cloud Manager with permissions
...
+
If you want to use a different AWS account for your backups, you must add the destination AWS account credentials in Cloud Manager, and add the permissions "s3:PutBucketPolicy" and "s3:PutBucketOwnershipControls" to the IAM role that provides Cloud Manager with permissions.
