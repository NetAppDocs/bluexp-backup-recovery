...
* Volume restore is supported for on-prem ONTAP systems; Cloud Volumes ONTAP systems are not currently supported.
* Directory/folder restore is not currently supported. 
* Backups can't be moved to archival storage.
* Backups can't be protected using DataLock and Ransomware protection.
====
