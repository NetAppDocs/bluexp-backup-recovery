...
[NOTE]
====
*FlexGroup restrictions*: 

* Full volume restore is supported for on-prem ONTAP systems; Cloud Volumes ONTAP systems are not currently supported.
...
* Backups can't be moved to archival storage.
* Backups can't use DataLock and Ransomware protection.
====
...
* File level restore is not supported when using the same account with different BlueXP systems in different subnets.
