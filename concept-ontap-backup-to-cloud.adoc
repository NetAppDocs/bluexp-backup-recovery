...
Cloud Backup can be used in a site with no internet connectivity (also known as "restricted" or "dark" sites) to back up volume data. Both volume and file restore are also supported in this configuration. In this case, you'll need to deploy the BlueXP Connector in the dark site. 
