...
** If you set a Snapshot policy with the daily, monthly, or weekly, etc. labels on the destination volume, these Snapshots will not be moved to the cloud as backup files. At this time you'll need to apply these labels to the source volume in order for Cloud Backup to back them up.
