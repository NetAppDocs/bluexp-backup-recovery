...
Cloud Backup can be used in a site with no internet connectivity (also known as an "offline" or "dark" site) to back up volume data. Both volume and file restore are also supported in this configuration. In this case, you'll need to deploy the BlueXP Connector in the dark site. 
...
* You can back up data from Cloud Volumes ONTAP systems installed in AWS and Azure commercial regions to Amazon S3 or Azure Blob. See link:task-backup-to-s3.html[Backing up Cloud Volumes ONTAP data to Amazon S3] and link:task-backup-to-azure.html[Backing up Cloud Volumes ONTAP data to Azure Blob].
