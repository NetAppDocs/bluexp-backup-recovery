...
summary: The BlueXP backup and recovery service provides backup and restore capabilities for protection and long-term archive of your ONTAP volume data. You can implement a 3-2-1 strategy where you have 3 additional copies of your data on 2 different storage systems along with 1 copy in the cloud.
---
...
[.lead]
The BlueXP backup and recovery service provides backup and restore capabilities for protection and long-term archive of your ONTAP volume data. You can implement a 3-2-1 strategy where you have 3 additional copies of your data on 2 different storage systems along with 1 copy in the cloud.
...
BlueXP backup and recovery leverages NetApp's SnapMirror data replication technology to automatically make sure the copies are fully synchronized at all times by creating Snapshot copies and transferring them to the backup locations.
...
== Supported ONTAP working environments and backup locations
...
You can restore ONTAP data from a backup file that resides in a secondary working environment or in object storage to the following working environments. Snapshot copies reside on the source working environment and can be restored only to that same system.
...
| *Object Storage* | *Secondary System* |
ifdef::aws[]
...
There are two types of costs associated with using BlueXP backup and recovery with ONTAP systems: resource charges and service charges. Both of these charges are for the backup to object portion of the service. There is no charge to create Snapshot copies or replicated volumes.
