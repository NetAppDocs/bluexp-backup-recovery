...
When necessary, you can restore an entire _volume_, a folder, or one or more _files_, from a backup to the same or different working environment.
...
* WORM/Compliance mode on an object store is only supported on Amazon S3 and StorageGRID at this time. This is known as the DataLock feature.
