...
* Governance
+
DataLock is set to _Governance_ mode where users with s3:BypassGovernanceRetention permissions (link:concept-cloud-backup-policies.html#requirements[see below]) can overwrite or delete backup files during the retention period. Ransomware protection is enabled.
...
* The DataLock option you select when activating Cloud Backup must be used for all backup policies for that cluster. 
